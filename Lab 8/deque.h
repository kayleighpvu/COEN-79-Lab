...
//Student work coded by Kayleigh Vu
//Lab 8 05/25/2023 - deque.h 
...
namespace coen79_lab8
{
    ...
    void deque<Item>::operator=(const deque<Item>& source) {
        ...
        // STUDENT WORK...
        block_pointers = new value_type* [source.bp_array_size];    //new array given the size of source
        block_pointers_end = source.block_pointers_end;             //set the end to whatever source's end is
        ...
        for (size_type bp_array_index = 0; bp_array_index < source.bp_array_size; ++bp_array_index)
        {
            ...
            else
            {
                ...
                // STUDENT WORK...
                if (*(source.first_bp) == (source.block_pointers[bp_array_index]))  //comments given above already, self-explanatory
                ...
                {
                    // STUDENT WORK...
                    //setting the front_ptr if address is the same with source's address of front_ptr
                    if (source.block_pointers[bp_array_index] + block_item_index == source.front_ptr) 
                        //assigning the front_ptr in current deque points to element of source deque
                        front_ptr = block_pointers[bp_array_index] + block_item_index;
    
                    //setting the back_ptr if address is the same with source's address of back_ptr
    ...
    void deque<Item>::clear () {
        ...
        // Clear the data blocks
        // STUDENT WORK...
        // iterating through to delete each one and set each place to NULL 
