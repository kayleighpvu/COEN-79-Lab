...
namespace coen79_lab5
{
    ...
    public:
        ...
        string(const char str[ ] = "");     //kay done
        string(char c);                     // NEW FUNCTION 1   //monica done
        string(const string& source);       //kay done
        ~string( );                         //monica done
        
        // MODIFICATION MEMBER FUNCTIONS
        void operator +=(const string& addend);     //monica done
        void operator +=(const char addend[ ]);     //monica done
        void operator +=(char addend);              //monica done  
        void reserve(size_t n);                     //monica done
        string& operator =(const string& source);   //monica done
        void insert(const string& source, unsigned int position);   //kayleigh done
        void dlt(unsigned int position, unsigned int num);      //monica done
        void replace(char c, unsigned int position);            //kayleigh done
        void replace(const string& source, unsigned int position);      //kayleigh done
        
        // CONSTANT MEMBER FUNCTIONS
        size_t length( ) const { return current_length; }   //given
        char operator [ ](size_t position) const;       //monica done
        int search(char c) const;                       //kayleigh done
        int search(const string& substring) const;      //monica done
        unsigned int count(char c) const;               //monica done
        
        // FRIEND FUNCTIONS
        friend std::ostream& operator <<(std::ostream& outs, const string& source);     //monica done
        friend bool operator ==(const string& s1, const string& s2);    //monica done
        friend bool operator !=(const string& s1, const string& s2);    //kayleigh done
        friend bool operator > (const string& s1, const string& s2);    //kayleigh done
        friend bool operator < (const string& s1, const string& s2);    //kayleigh done 
        friend bool operator >=(const string& s1, const string& s2);    //kayleigh done
        friend bool operator <=(const string& s1, const string& s2);    //kayleigh done
    ...
    // NON-MEMBER FUNCTIONS for the string class
    string operator +(const string& s1, const string& s2);              //kayleigh done
    string operator +(const string& s1, const char addend[ ]);          //kayleigh done
    std::istream& operator >> (std::istream& ins, string& target);      //kayleigh done
}
